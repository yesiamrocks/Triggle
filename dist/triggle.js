(function(factory) {
  ...
  (function() {
    ...
    function matchesKeyFilter(keyFilter, event) {
      ...
      return rules.some((rule) => {
        if (rule.includes("+")) {
          ...
          const match = key === event.key.toLowerCase() && (mod === "ctrl" && event.ctrlKey || mod === "shift" && event.shiftKey || mod === "alt" && event.altKey);
          if (window.__CA_DEBUG)
            console.log(
              `[ca-trigger] modifier match: ${match}, rule: ${rule}, event.key: ${event.key}`
            );
          return match;
          ...
          if (window.__CA_DEBUG)
            console.log(
              `[ca-trigger] wildcard match: ${match}, rule: ${rule}, event.key: ${event.key}`
            );
          return match;
        } else {
          ...
          if (window.__CA_DEBUG)
            console.log(
              `[ca-trigger] exact match: ${match}, rule: ${rule}, event.key: ${event.key}`
            );
          return match;
        }
      });
    ...
    function initTriggerAnimations() {
      ...
      elements.forEach((el) => {
        const triggers = el.getAttribute("data-ca-trigger").split(",").map((t) => t.trim());
